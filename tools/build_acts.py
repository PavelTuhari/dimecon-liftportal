...
ACT_DEFS = [
    ...
    {
        "id": "10_upravlencheskie_moduli",
        "num": "10",
        "title": "Управленческие модули: проекты, продажи, закупки, счета, таблицы, документы",
        "lead": "Шесть модулей по образцу Odoo для застройщика, перенесённые на язык компании, "
                "которая сдаёт технику и выполняет работы.",
        "scope": ["проекты: иерархия работ, подзадачи, зависимости, вехи, повторяющиеся задачи",
                  "учёт времени и рентабельность объекта: план против факта",
                  "шаблоны объектов с копированием задач и связей",
                  "продажи: смета со скидкой и НДС, допработы, принятие клиентом по ссылке, счёт и аванс",
                  "закупки: запрос цены, подтверждение, частичная и полная приёмка, счёт поставщика, оплата",
                  "поставщики: сроки, просрочки, анализ закупок; правила пополнения склада",
                  "счета: платежи и статусы, просрочка, дебиторка по срокам, кредит-ноты, регулярные счета",
                  "таблицы: формулы, живые данные компании, сводные срезы, диаграммы, выгрузка в Excel",
                  "документы: рабочие пространства, теги, версии, запросы файлов по ссылке, автоправила",
                  "изоляция всех новых данных между компаниями"],
        "checks": checks_from("test_erp.log"),
        "screens": shots("90_erp_projects", "91_erp_project", "92_erp_tasks", "93_erp_sales", "94_erp_sale",
                         "104_erp_sales_accept", "95_erp_purchase", "96_erp_vendors", "97_erp_reorder",
                         "98_erp_invoices", "99_erp_invoice", "100_erp_plans", "101_erp_sheets",
                         "102_erp_sheet", "103_erp_docs"),
        "verdict": "Годен",
        "summary": ["по объекту видно план и факт: выручка, закупки, труд по табелям, маржа",
                    "смета считается построчно, клиент принимает предложение по ссылке и выбирает допработы",
                    "закупка проходит путь от запроса цены до оплаты, приёмка пополняет склад",
                    "счёт меняет статус сам по платежам и сроку, дебиторка разложена по срокам",
                    "таблица считает формулы и подставляет живые данные компании прямо в ячейки"],
        "limits": ["Электронная подпись, онлайн-оплата и распознавание счетов не реализованы — "
                   "они требуют договоров с платёжным провайдером и сервисом подписи.",
                   "Совместное редактирование таблицы в реальном времени не поддерживается: "
                   "расчёт выполняется на сервере при сохранении.",
                   "Складской учёт ограничен остатком товара: партии, серийные номера и "
                   "несколько складов в объём работ не входили."],
    },
    {
        "id": "11_nastroyki_zapuska",
        "num": "11",
        "title": "Настройки запуска компании и их применение",
        "lead": "Пять пунктов из списка «что дальше» настраиваются в кабинете, и система сразу "
                "начинает по ним работать.",
        "scope": ["грузовые таблицы производителя: разбор файла, проверка данных, замена ориентировочной кривой",
                  "переключение подбора на паспортные строки и снятие предупреждения об оценке",
                  "прайс: ставки и минимальные смены по машинам, зоны подачи, коэффициенты, допуслуги, скидки",
                  "влияние прайса на расчёт заказа",
                  "реквизиты банка и подписи в счетах, срок оплаты и префиксы номеров",
                  "почтовый сервер компании и проверочное письмо",
                  "собственный домен: подсказка по DNS, проверка привязки, основной адрес",
                  "скрипты, применяющие те же настройки из командной строки",
                  "права: настройки доступны владельцу и закрыты для чужих компаний"],
        "checks": checks_from("test_settings.log"),
        "screens": shots("110_setup_index", "111_setup_charts", "112_setup_pricing",
                         "113_setup_company", "114_setup_domain"),
        "verdict": "Годен",
        "summary": ["после загрузки паспортной таблицы подбор считает по ней, а не по оценке с запасом 15 %",
                    "зоны подачи и коэффициенты из формы сразу меняют сумму заказа",
                    "IBAN, код НДС и подпись руководителя печатаются в счёте, срок оплаты берётся из настроек",
                    "домен проверяется по DNS и по ответу сайта, подсказки по записям выдаются автоматически",
                    "то же самое доступно скриптами: отчёт готовности, загрузка таблиц, прайс, реквизиты, домен, ERP"],
        "limits": ["Проверка домена выполняется с сервера платформы: если запись DNS ещё не разошлась, "
                   "проверку нужно повторить позже.",
                   "Сертификат HTTPS для своего домена выпускается на сервере после успешной проверки — "
                   "это отдельный шаг администратора платформы.",
                   "Доступы к OfficePlus и Oracle на стенде не заполнены: скрипт обмена показывает, "
                   "что подключение не настроено, а не работает вхолостую."],
    },
    {
        "id": "08_integracii",
        "num": "8",
...
SLIDES = [
    ...
    {"kind": "shot", "img": "91_erp_project", "title": "Объект целиком", "tall": True,
     "text": "План и факт по стройке: работы с зависимостями и вехами, списанные часы, закупки, "
             "счета и маржа — на одном экране."},
    {"kind": "shot", "img": "94_erp_sale", "title": "Смета и коммерческое предложение", "tall": True,
     "text": "Строки из каталога техники и услуг, скидка, НДС, допработы на выбор клиента и маржа "
             "по каждому предложению. Клиент принимает по ссылке, счёт выставляется одной кнопкой."},
    {"kind": "shot", "img": "98_erp_invoices", "title": "Деньги под контролем", "tall": True,
     "text": "Счета сами меняют статус по платежам и сроку, дебиторка разложена по срокам, "
             "напоминания уходят должникам, регулярные счета выставляются по расписанию."},
    {"kind": "shot", "img": "95_erp_purchase", "title": "Закупки и поставщики",
     "text": "Запрос цены, подтверждение, приёмка на склад, счёт поставщика и оплата. "
             "По каждому поставщику видно объём, сроки и долю опозданий."},
    {"kind": "shot", "img": "102_erp_sheet", "title": "Таблицы с живыми данными", "tall": True,
     "text": "Формулы как в Excel плюс функции, читающие базу компании: выручка, долг, парк, часы. "
             "Сводные срезы, диаграммы и выгрузка в Excel."},
    {"kind": "shot", "img": "103_erp_docs", "title": "Документы на месте", "tall": True,
     "text": "Рабочие пространства, теги и версии, запрос недостающего документа по ссылке "
             "и правила, раскладывающие входящие файлы автоматически."},
    ...
    {"kind": "shot", "img": "110_setup_index", "title": "Что осталось настроить — видно в кабинете", "tall": True,
     "text": "Пять пунктов запуска с состоянием и объяснением, зачем каждый нужен. Заполняете — "
             "система сразу считает по вашим данным: паспортные таблицы, прайс, реквизиты, ERP, домен."},
